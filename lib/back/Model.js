...
var Model = (function () {
  ...
  _createClass(Model, [{
    ...
    value: function asJSON() {
      var out = {},
          respondsTo = function respondsTo(value) {
        return value && value['asJSON'];
      };

      for (var key in this.map) {
        var value = this.map[key];
        if (Array.isArray(value) && respondsTo(value[0])) out[key] = value.map(function (v) {
          return v.asJSON();
        });else if (respondsTo(value)) out[key] = value.asJSON();else out[key] = value;
      }
